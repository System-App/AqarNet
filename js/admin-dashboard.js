...
export async function renderDashboard(main) {
  ...
  const rerender = () => {
    ...
    if (employees.length === 0) {
      listEl.appendChild(el('div.empty', {}, [
        ...
        el('button.btn.btn--primary', { onclick: () => window.AqarNet.navigateTo('settings') }, 'چوون بۆ ڕێکخستنەکان'),
      ]));
      return;
    }
  ...
  window.AqarNet_registerCleanup(cleanup);
}
...
async function openTodayEdit(emp, settings, today) {
  try {
    ...
    console.error('[Aqar Net] Open today edit error:', err);
