...
export async function renderAttendance(main) {
  ...
  async function load() {
    ...
    try {
      ...
      console.error('[Aqar Net] Attendance load error:', err);
      content.innerHTML = '';
...
function buildSummaryCard(s) {
  return el('div.summary-card', {
    onclick: () => { haptic(8); window.AqarNet.navigateTo('detail', { employeeId: s.employee.id }); },
  }, [
...
function buildDetailHead(employee, startDate, endDate, totals) {
  return el('div.detail-head', {}, [
    el('div.detail-head__top', {}, [
      el('button.detail-head__back', { onclick: () => window.AqarNet.navigateTo('attendance'), 'aria-label': 'گەڕانەوە',
...
export function openEditDay(employee, date, day, settings, reload) {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error('[Aqar Net] Edit day error:', err);
