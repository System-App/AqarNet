...
export async function renderSettings(main) {
  const settings = window.AqarNet.getSettings();
  const prefs = window.AqarNet.getPrefs();
  ...
  wrap.appendChild(buildSection(svgPalette(), 'ڕووکار', [
    buildToggle('ڕووکاری تاریک', 'گۆڕینی ڕووکار بۆ ڕەنگی تاریک.',
      prefs.darkMode ?? false, (c) => window.AqarNet.setPref('darkMode', c)),
  ...
  wrap.appendChild(buildSection(svgKey(), 'هەژمار', [
    ...
    el('button.btn.btn--danger.btn--block', {
      onclick: async () => {
        ...
        if (ok) await window.AqarNet.logout();
      },
  ...
  wrap.appendChild(el('div', { style: { textAlign: 'center', padding: '12px', color: 'var(--ink-faint)', fontSize: 'var(--text-xs)' } }, [
    el('div', { style: { fontWeight: '700', color: 'var(--ink-soft)' } }, 'Aqar Net'),
    el('div', { style: { marginTop: '2px' } }, 'وەشانی 1.3.0'),
  ]));
...
function buildWorkHours(settings) {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      await window.AqarNet.updateSettings({ workStart: ws, workEnd: we, gracePeriod: grace });
      toast('کاتی کار پاشەکەوتکرا', 'success');
      haptic(15);
    } catch (err) {
      console.error('[Aqar Net] Save work hours error:', err);
...
function buildZoneSection(settings) {
  ...
  const updateMapMarker = () => {
    ...
    else circle = L.circle([lat, lng], { radius, color: '#1A2A45', fillColor: '#E0A43B', fillOpacity: 0.2, weight: 2 }).addTo(map);
  };
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    saveBtn.disabled = true;
    try {
      await window.AqarNet.updateSettings({ zone: { enabled: enableInput.checked, lat, lng, radius } });
      toast('ناوچەی دەوام پاشەکەوتکرا', 'success');
      haptic(15);
    } catch (err) {
      console.error('[Aqar Net] Save zone error:', err);
      toast('کێشە لە پاشەکەوتکردن', 'error');
    } finally {
      saveBtn.disabled = false;
    }
...
function buildAutoCheckout(settings) {
  ...
  input.addEventListener('change', async () => {
    haptic(10);
    try {
      await window.AqarNet.updateSettings({ autoCheckout: input.checked });
...
function openAddHoliday(onSaved) {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error('[Aqar Net] Add holiday error:', err);
...
async function handleRestore(file) {
  try {
    ...
    if (!payload || payload.app !== 'aqar-net-attendance') {
      toast('ئەم فایلە بۆ سیستەمی Aqar Net نییە', 'error');
      return;
    }
    ...
    setTimeout(() => window.AqarNet.navigateTo('settings'), 800);
  } catch (err) {
    console.error('[Aqar Net] Restore error:', err);
...
function buildFontControl(prefs) {
  ...
  const apply = (scale) => { window.AqarNet.setPref('fontScale', scale); render(scale); };
  let timer;
  slider.addEventListener('input', () => {
    const scale = parseInt(slider.value, 10) / 100;
    window.AqarNet.setPref('fontScale', scale);
