/**
 * app.js — Main router & application shell controller for Aqar Net
 *
 * Responsibilities:
 ...
 *   • Provide a global window.AqarNet API used by all view modules
...
window.AqarNet_registerCleanup = (fn) => { state.routeCleanup = fn; };

// Local UI preferences (per-device, not synced)
const PREFS_KEY = 'aqar-net-prefs';
function loadPrefs() {
...
function applyTheme(dark) {
  ...
  if (meta) meta.setAttribute('content', dark ? '#0E1626' : '#1A2A45');
}
...
// ──────────────────────────────────────────────────────────────────
// GLOBAL API — used by all view modules via window.AqarNet
// ──────────────────────────────────────────────────────────────────
window.AqarNet = {
  // state getters
  getUser: () => state.user,
...
async function navigateTo(route, params = {}) {
  ...
  if (state.routeCleanup) {
    try { state.routeCleanup(); } catch (e) { console.warn('[Aqar Net] Cleanup error:', e); }
  ...
  try {
    ...
    console.error('[Aqar Net] Route render error:', err);
...
async function setupSession(user) {
  ...
  runAutoCheckout(state.settings, autoScope).then((n) => {
    if (n > 0) console.log(`[Aqar Net] Auto-checkout filled ${n} record(s)`);
  }).catch((e) => console.warn('[Aqar Net] Auto-checkout error:', e));
...
function showAppShell() {
  ...
  if (nameEl) nameEl.textContent = 'Aqar Net';
}
...
function registerServiceWorker() {
  ...
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('sw.js')
      ...
      .catch((e) => console.warn('[Aqar Net] SW registration failed:', e));
  });
}
...
function setupInstallPrompt() {
  ...
  const dismissed = localStorage.getItem('aqar-net-install-dismissed') === '1';
  ...
  const iosDismissed = localStorage.getItem('aqar-net-ios-dismissed') === '1';
  ...
  if (close) close.addEventListener('click', (e) => {
    ...
    hideBanner(); localStorage.setItem('aqar-net-install-dismissed', '1');
  });
  if (iosClose) iosClose.addEventListener('click', (e) => {
    e.preventDefault(); e.stopPropagation();
    hideIos(); localStorage.setItem('aqar-net-ios-dismissed', '1');
  });
...
window.AqarNet.navigateTo = navigateTo;
