...
export async function renderReports(main) {
  ...
  async function refresh() {
    ...
    try {
      ...
      console.error('[Aqar Net] Report error:', err);
      preview.innerHTML = '';
...
function buildPreview(summaries, startDate, endDate) {
  ...
  return el('div.report-preview', {}, [
    el('div.report-preview__head', {}, [
      el('div.report-preview__logo', {}, [el('img', { src: 'icons/icon-192.png', alt: 'Aqar Net' })]),
      el('div.report-preview__title', {}, 'Aqar Net — ڕاپۆرتی دەوام'),
...
<head>
<meta charset="UTF-8">
<title>Aqar Net — ڕاپۆرتی دەوام</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;600;700;800&display=swap');
  * { margin:0; padding:0; box-sizing:border-box; }
  body { font-family:'Vazirmatn',sans-serif; color:#232B20; padding:32px; direction:rtl; }
  .header { text-align:center; border-bottom:3px solid #1A2A45; padding-bottom:16px; margin-bottom:24px; }
  .header h1 { font-size:24px; color:#1A2A45; }
  ...
  .summary .v { font-size:22px; font-weight:800; color:#1A2A45; direction:ltr; }
  .summary .l { font-size:12px; color:#5C6B53; }
  table { width:100%; border-collapse:collapse; font-size:13px; }
  th,td { padding:10px 8px; text-align:right; border-bottom:1px solid #E7E0D4; }
  th { background:#1A2A45; color:#F5EFE3; font-weight:700; font-size:12px; }
  ...
  tr.total td { border-top:2px solid #1A2A45; }
...
<body>
  <div class="header">
    <h1>Aqar Net — ڕاپۆرتی دەوامی کارمەندان</h1>
  ...
  <div class="footer">Aqar Net © 2026 — سیستەمی دەوام</div>
</body>
</html>`;
  ...
  const triggerPrint = () => {
    ...
    try {
      ...
      console.warn('[Aqar Net] Print error:', e);
      cleanup();
...
function exportCSV(summaries, startDate, endDate) {
  ...
  const filename = `aqar-net-report-${startDate}_to_${endDate}.csv`;
